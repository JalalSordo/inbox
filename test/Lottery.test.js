...
describe("Lottery Smart contract tests", () => {
  it("deployable", () => {
    assert.ok(deployedContract.options.address);
    assert.ok(deployedContract.methods.manager().call());
  });
  ...
  it("Enter lottery", async () => {
    ...
    await deployedContract.methods
      .enter()
      .send({ from: fetchedAcccounts[1], value: 2 });
    await deployedContract.methods
      .enter()
      .send({ from: fetchedAcccounts[2], value: 2 });
    await deployedContract.methods
      .enter()
      .send({ from: fetchedAcccounts[3], value: 2 });
    await deployedContract.methods
      .enter()
      .send({ from: fetchedAcccounts[4], value: 2 });
  ...
  it("requires a minimum value to enter lottery", async () => {
    try {
      await deployedContract.methods
        .enter()
        .send({ from: fetchedAcccounts[1], value: 1 });
  ...
  it("only manager allowed to pick a winner", async () => {
    try {
      await deployedContract.methods
        .enter()
        .send({ from: fetchedAcccounts[1], value: 2 });
  ...
  it("pick a winner", async () => {
    ...
    const winner = await deployedContract.methods.lastWinner().call();
    assert.ok(winner)
